...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Asus\\Documents\\pretest_fmri\\Guess_fMRI\\guess.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    iti_learning_cross = visual.TextStim(win=win, name='iti_learning_cross',
        text='+',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    thisExp.addLoop(trial_iti)  # add the loop to the experiment
    thisTrial_iti = trial_iti.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisTrial_iti.rgb)
    if thisTrial_iti != None:
        for paramName in thisTrial_iti:
            globals()[paramName] = thisTrial_iti[paramName]
    
    for thisTrial_iti in trial_iti:
        currentLoop = trial_iti
        thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
        # abbreviate parameter names if possible (e.g. rgb = thisTrial_iti.rgb)
        if thisTrial_iti != None:
            for paramName in thisTrial_iti:
        ...
        # if trial has changed, end Routine now
        if isinstance(trial_iti, data.TrialHandler2) and thisTrial_iti.thisN != trial_iti.thisTrial.thisN:
            continueRoutine = False
        ...
        routineTimer.reset()
    # completed 1.0 repeats of 'trial_iti'
    ...
    while continueRoutine:
        ...
        if instruction5_key.status == STARTED and not waitOnFlip:
            theseKeys = instruction5_key.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
    ...
    for thisPractice_trial in practice_trials:
        ...
        guess_cond = practice_list.pop()[0]
        print(guess_cond)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    ...
    for thisLearning_trial in learning_trials:
        ...
        # create an object to store info about Routine setup_trial
        setup_trial = data.Routine(
            name='setup_trial',
            components=[],
        )
        setup_trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from set_iti_guess_stim
        guess_cond = guess_list.pop()[0]
        this_iti = iti_list.pop()[0]
        print(guess_cond, this_iti)
        # store start times for setup_trial
        setup_trial.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        setup_trial.tStart = globalClock.getTime(format='float')
        setup_trial.status = STARTED
        thisExp.addData('setup_trial.started', setup_trial.tStart)
        setup_trial.maxDuration = None
        # keep track of which components have finished
        setup_trialComponents = setup_trial.components
        for thisComponent in setup_trial.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        if isinstance(learning_trials, data.TrialHandler2) and thisLearning_trial.thisN != learning_trials.thisTrial.thisN:
            continueRoutine = False
        setup_trial.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                setup_trial.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in setup_trial.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in setup_trial.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for setup_trial
        setup_trial.tStop = globalClock.getTime(format='float')
        setup_trial.tStopRefresh = tThisFlipGlobal
        thisExp.addData('setup_trial.stopped', setup_trial.tStop)
        ...
        while continueRoutine:
            ...
            # *iti_learning_cross* updates
            
            # if iti_learning_cross is starting this frame...
            if iti_learning_cross.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                iti_learning_cross.frameNStart = frameN  # exact frame index
                iti_learning_cross.tStart = t  # local t and not account for scr refresh
                iti_learning_cross.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(iti_learning_cross, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'iti_learning_cross.started')
                # update status
                iti_learning_cross.status = STARTED
                iti_learning_cross.setAutoDraw(True)
            
            # if iti_learning_cross is active this frame...
            if iti_learning_cross.status == STARTED:
                # update params
                pass
            
            # if iti_learning_cross is stopping this frame...
            if iti_learning_cross.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > iti_learning_cross.tStartRefresh + this_iti-frameTolerance:
                    # keep track of stop time/frame for later
                    iti_learning_cross.tStop = t  # not accounting for scr refresh
                    iti_learning_cross.tStopRefresh = tThisFlipGlobal  # on global time
                    iti_learning_cross.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'iti_learning_cross.stopped')
                    # update status
                    iti_learning_cross.status = FINISHED
                    iti_learning_cross.setAutoDraw(False)
        ...
        # the Routine "iti_learning" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
    ...
    print(guess_list)
    # Run 'End Experiment' code from load_iti
    print(guess_list)
